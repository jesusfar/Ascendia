...
# --no-defaults: ignora /etc/mysql (que fuerza log-error=/var/log/mysql y user=mysql,
# inaccesibles para uid 1000). mysqlx-socket en dir escribible para el X-plugin.
mysqld --no-defaults --datadir="$DATADIR" --socket="$SOCK" --port=3306 --bind-address=127.0.0.1 \
       --pid-file="$RUNDIR/mysqld.pid" --innodb-buffer-pool-size=128M \
       --mysqlx-socket="$RUNDIR/mysqlx.sock" --secure-file-priv="" &

# esperar a que levante
until mysqladmin --no-defaults --socket="$SOCK" ping --silent 2>/dev/null; do sleep 1; done

# crear base + usuario de la app (native_password para evitar lios de auth por TCP)
mysql --no-defaults --socket="$SOCK" -uroot <<'SQL'
...
mysql --no-defaults --socket="$SOCK" -uroot ascendia < database/ascendia_schema.sql
