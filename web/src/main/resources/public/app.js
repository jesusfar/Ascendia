...
/* ---------------------------------------------------------------------------
   1. CARGA DE DATOS
   Los "faros" de oportunidades vienen de la API Java (RF11): GET /api/oportunidades,
   que reutiliza los DAO/JDBC del prototipo y los sirve desde MySQL.
   Se conserva el array embebido como RESPALDO: si la API no responde (demo offline
   o servidor caído), el globo sigue funcionando sin tocar el resto de la app.
--------------------------------------------------------------------------- */
async function cargarDatos() {
  try {
    ...
    console.warn('API no disponible, uso datos de respaldo:', e);
    return [
...
/* Nombres de lugares en español.
   Natural Earth trae las etiquetas en inglés. Para países y provincias usamos
   los campos NAME_ES / name_es del propio geojson (acentos correctos y, p. ej.,
   "Islas Malvinas" en vez de "Falkland Is."). Para ciudades el dataset "simple"
   no trae campo en español, así que aplicamos este override sobre las más
   visibles. El archivo se sirve y declara como UTF-8 (meta charset). */
const OVERRIDE_ES = {
  "Algiers": "Argel",
  "Athens": "Atenas",
  "Baghdad": "Bagdad",
